    ...
    def test_manual_data_version_override_triggers_downstream_update(
        ...
        with graph.use():
            ...
            with DuckDBMetadataStore(db_path, auto_create_tables=True) as store:
                ...
                assert parent_current[METAXY_DATA_VERSION].to_list() == [
                    "hash_content_a_v1",
                    "hash_content_b_v1",
                    "hash_content_c_v1",
                ]
                ...
                child_df_v1 = pl.DataFrame(
                    {
                        "sample_uid": ["sample_a", "sample_b", "sample_c"],
                        "derived": ["derived_a", "derived_b", "derived_c"],
                        METAXY_PROVENANCE_BY_FIELD: [
                            {"derived": "child_prov_a"},
                            {"derived": "child_prov_b"},
                            {"derived": "child_prov_c"},
                        ],
                    }
                )
                store.write_metadata(ChildFeature, nw.from_native(child_df_v1))

                # Step 4: Verify initial state - no updates needed
                increment_before_change = store.resolve_update(
                    ChildFeature, lazy=False
                )
    ...
    def test_data_version_vs_provenance_independence(self, tmp_path: Path) -> None:
        ...
        with graph.use():
            ...
            with DuckDBMetadataStore(db_path, auto_create_tables=True) as store:
                ...
                result = store.read_metadata(RootFeature).collect().to_polars()
    ...
    def test_partial_data_version_update_selective_recomputation(
        ...
        with graph.use():
            ...
            with DuckDBMetadataStore(db_path, auto_create_tables=True) as store:
                ...
                store.write_metadata(DownstreamFeature, nw.from_native(downstream_df))
    ...
    def test_root_feature_with_custom_data_version(self, tmp_path: Path) -> None:
        ...
        with graph.use():
            ...
            with DuckDBMetadataStore(db_path, auto_create_tables=True) as store:
                ...
                increment = store.resolve_update(RootFeature, samples=samples, lazy=False)
