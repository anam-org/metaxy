...
def test_field_mapping_on_feature_dep(graph: FeatureGraph):
    ...
    plan_default = graph.get_feature_plan(FeatureKey(["downstream_default"]))
...
def test_field_mapping_with_suffix_matching(graph: FeatureGraph):
    ...
    plan = graph.get_feature_plan(FeatureKey(["downstream_suffix"]))
    ...
    assert FieldKey(["audio", "french"]) in french_deps[FeatureKey(["upstream_audio"])]
...
def test_field_mapping_with_explicit_all(graph: FeatureGraph):
    ...
    plan = graph.get_feature_plan(FeatureKey(["downstream_all"]))
    field_deps = plan.field_dependencies
...
def test_explicit_deps_override_field_mapping(graph: FeatureGraph):
        ...
        spec=SampleFeatureSpec(
            ...
            fields=[
                ...
                FieldSpec(
                    key="data",
                    deps=[
                        FieldDep(feature="upstream_x", fields=[FieldKey(["metadata"])]),
    ...
    plan = graph.get_feature_plan(FeatureKey(["downstream_explicit"]))
    field_deps = plan.field_dependencies
...
def test_field_mapping_with_exclusions(graph: FeatureGraph):
    ...
    plan = graph.get_feature_plan(FeatureKey(["downstream_exclusions"]))
    field_deps = plan.field_dependencies
...
def test_no_field_mapping_specified(graph: FeatureGraph):
    ...
    plan = graph.get_feature_plan(FeatureKey(["simple_downstream"]))
    field_deps = plan.field_dependencies
