...
def test_safe_env_settings_source_catches_attribute_error():
    ...
    result = source.field_is_complex(cast(Any, mock_field))
    print(f"field_is_complex result: {result}")
    assert result is True

    # SafeEnvSettingsSource._field_is_complex should catch the error
    result_tuple = source._field_is_complex(cast(Any, mock_field))
    ...
    assert source.decode_complex_value("test_field", cast(Any, mock_field), "test_value") == "test_value"
...
def test_safe_env_settings_source_normal_behavior():
    ...
    assert source.field_is_complex(cast(Any, real_field)) is False
...
def test_metaxy_config_uses_safe_source(monkeypatch):
    ...
    sources = MetaxyConfig.settings_customise_sources(MockSettings, mock_init, mock_env, mock_dotenv, mock_file)
