...
def test_code_version_single_field() -> None:
    graph = FeatureGraph()
    with graph.use():
...
def test_code_version_multiple_fields_order_invariant() -> None:
    graph = FeatureGraph()
    with graph.use():
        field_items = [
            (FieldKey(["frames"]), 1),
        ...
        def build_fields(items: list[tuple[FieldKey, int]]) -> list[FieldSpec]:
            return [
                FieldSpec(key=key, code_version=code_version)
...
def test_code_version_changes_when_field_changes() -> None:
    graph = FeatureGraph()
    with graph.use():
...
def test_code_version_ignores_dependency_versions() -> None:
    graph = FeatureGraph()
    with graph.use():
...
def test_code_version_deterministic_per_class() -> None:
    graph = FeatureGraph()
    with graph.use():
        feature_cls = _build_feature(
