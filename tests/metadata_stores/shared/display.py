...
class DisplayTests:
    ...
    def test_name_is_none_by_default(self, store: MetadataStore):
        assert store.name is None

    def test_display_without_name(self, store: MetadataStore):
        display = store.display()
        assert not display.startswith("[")
        assert store.__class__.__name__ in display

    def test_name_property_returns_configured_name(self, named_store: MetadataStore):
        assert named_store.name is not None

    def test_display_without_name_even_when_configured(self, named_store: MetadataStore):
        display = named_store.display()
        assert not display.startswith("[")
        assert named_store.__class__.__name__ in display

    def test_repr_includes_name(self, named_store: MetadataStore):
