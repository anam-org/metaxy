"""Resolve-update test pack for metadata stores.

Tests resolve_update behavior including:
...
- Idempotency
- Filter key types
- Expansion / aggregation lineage
- Optional dependencies
- Staleness predicates
- compute_provenance for eager/lazy frames
"""
...
class ResolveUpdateTests:
    """Tests for resolve_update, staleness predicates, and compute_provenance."""

    # ============= ROOT FEATURES (NO UPSTREAM) =============
    ...
    # ============= DOWNSTREAM FEATURES (WITH UPSTREAM) =============
    ...
    # ============= EXPANSION LINEAGE WITH MULTIPLE MATERIALIZATIONS =============
        ...
        with fallback_store.open("w"), local_store.open("w"), graph.use():
            ...
            with local_only_store.open("w"):
                ...
                assert removed_count == 80, (
                    f"Expected 80 orphaned (100 downstream - 20 upstream), but got {removed_count}."
                )
    ...
    def test_resolve_update_optional_dependencies(
        ...
        graph: FeatureGraph,
        store: MetadataStore,
        optional_dep_config: FeaturePlanOutput,
    ) -> None:
        ...
        graph, upstream_features, child_plan = optional_dep_config
        child_key = child_plan.feature.key
    ...
    def test_optional_dependency_null_handling_and_provenance_stability(
        ...
        try:
            with store.open("w"):
                ...
                store.write(RequiredParent, required_df)
                ...
                store.write(OptionalParent, optional_df)
                ...
                store.write(ChildFeature, initial_downstream)
                ...
                store.write(OptionalParent, optional_df_v2)

                # Resolve after optional parent change
                increment_v2 = store.resolve_update(ChildFeature)
    ...
    def test_all_optional_deps_outer_join_behavior(
        ...
        try:
            with store.open("w"):
                ...
                store.write(OptionalA, optional_a_df)
                ...
                store.write(OptionalB, optional_b_df)
    ...
    # ============= AGGREGATION LINEAGE FIELD-LEVEL ISOLATION =============
    ...
    def test_aggregation_lineage_field_level_provenance_isolation(
        ...
        try:
            with store.open("w"):
                ...
                store.write(SensorReadings, upstream_v1)
                ...
                store.write(SensorReadings, upstream_v2)

                # Resolve after upstream change
                increment_v2 = store.resolve_update(HourlyStats)
    ...
    def test_aggregation_lineage_field_level_provenance_definition_change(
        ...
        with FeatureGraph().use():
            ...
            try:
                with store.open("w"):  # Store ops inside FeatureGraph context!
                    store.write(SensorReadingsV1, upstream_v1)
        ...
        with FeatureGraph().use():
            ...
            try:
                with store.open("w"):  # Store ops inside FeatureGraph context!
                    store.write(SensorReadingsV2, upstream_v2)
    ...
    def test_predicate_marks_matching_records_as_stale(self, store: MetadataStore) -> None:
    ...
    def test_multiple_predicates_are_ored(self, store: MetadataStore) -> None:
    ...
    def test_predicates_dont_duplicate_version_stale(self, store: MetadataStore) -> None:
        ...
        with graph.use(), store.open("w"):
            ...
            store.write(UpstreamFeature, upstream_data_v2)
    ...
    def test_predicates_dont_duplicate_new(self, store: MetadataStore) -> None:
    ...
    def test_skip_comparison_bypasses_predicates(self, store: MetadataStore) -> None:
    ...
    def test_first_run_no_current_metadata(self, store: MetadataStore) -> None:
    ...
    def test_lazy_mode(self, store: MetadataStore) -> None:
    ...
    def test_global_filters_scope_predicates(self, store: MetadataStore) -> None:
    ...
    def test_no_predicates_default_behavior(self, store: MetadataStore) -> None:
    ...
    def test_compute_provenance_eager_frame_computes_correctly(self, store: MetadataStore) -> None:
    ...
    def test_compute_provenance_lazy_frame_computes_correctly(self, store: MetadataStore) -> None:
