...
def test_datasink_with_multiple_blocks_sequential(
    ray_config: mx.MetaxyConfig,
    ray_context,
    delta_store: DeltaMetadataStore,
):
    """Test that MetaxyDatasink handles multiple data blocks with sequential writes.

    When writing multiple blocks to a Delta store, concurrent writes can cause conflicts.
    Using concurrency=1 ensures sequential writes and avoids race conditions.
    """
    from metaxy.ext.ray.datasink import MetaxyDatasink
    ...
    # Use concurrency=1 to avoid Delta version conflicts when writing multiple blocks
    ds.write_datasink(datasink, concurrency=1)
