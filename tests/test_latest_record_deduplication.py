...
def test_metaxy_created_at_deduplication_in_resolve_update(tmp_path: Path):
    ...
    with graph.use():
        ...
        with DuckDBMetadataStore(db_path, auto_create_tables=True) as store:
            ...
            first_records = store.read_metadata(TestFeature, current_only=False).collect().to_polars()
            first_timestamp = first_records["metaxy_created_at"][0]
            ...
            all_records = store.read_metadata(TestFeature, current_only=False).collect().to_polars()
            assert len(all_records) == 3
            
            # Read current only - should have 2 records (latest s1, s2)
            current_records = store.read_metadata(TestFeature, current_only=True).collect().to_polars()
            ...
            s2_time = current_records.filter(pl.col("sample_uid") == "s2")["metaxy_created_at"][0]
