...
Metaxy migrations live alongside your code in `.metaxy/migrations/` and can be written in either YAML (`.yaml`) or Python (`.py`). Both formats participate in the same parent chain, so you can freely mix them within the same project.
...
- **Python migrations** provide a full programming surface for conditional logic, helper methods, and custom operations. The `PythonMigration` base class lets you define migrations as normal Python classes and return strongly typed operations:
