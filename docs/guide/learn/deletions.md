...
from metaxy.models.types import CascadeMode, FeatureKey

# Get deletion order (leaf-first for hard deletes)
graph = FeatureGraph.get_active()
order = graph.get_cascade_deletion_order(
    FeatureKey(["video", "raw"]),
    CascadeMode.DOWNSTREAM,  # dependents first (leaf → root)
)
# Returns: [FeatureKey(["video", "faces"]), FeatureKey(["video", "chunk"]), FeatureKey(["video", "raw"])]
...
- `CascadeMode.DOWNSTREAM`: Returns dependents first (leaf → root). Use for hard deletions to avoid foreign key violations.
- `CascadeMode.UPSTREAM`: Returns dependencies first (root → leaf). Less common for deletions.
- `CascadeMode.BOTH`: Returns all connected features in dependency order.
...
- `cascade`: Deletion direction. Use uppercase enum names in Dagster run_config: `"NONE"`, `"DOWNSTREAM"`, `"UPSTREAM"`, `"BOTH"`

**Filter behavior with cascade deletion:**

When using cascade deletion with filters, the same filter expressions are applied to all features in the cascade. For example, deleting `VideoRaw` with filter `video_id='v1'` and `cascade=DOWNSTREAM` will:

1. Delete VideoChunk rows where `video_id='v1'`
2. Delete FaceRecognition rows where `video_id='v1'`
3. Delete VideoRaw rows where `video_id='v1'`

This ensures referential integrity when filters select a subset of rows. If you need to delete all dependent features regardless of filter matches, perform the deletion without filters.
