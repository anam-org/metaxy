...
**Filter behavior in cascade deletion:**

When using cascade deletion with filters, the same filter expressions are evaluated in the context of each feature in the cascade (using that feature's own columns). For example, deleting `video/raw` with filter `video_id='v1'` and `cascade=downstream` will:

1. Delete `VideoChunk` rows where `video_id='v1'`
2. Delete `FaceRecognition` rows associated with those deleted `VideoChunk` rows (for example, via `chunk_id`)
3. Delete `VideoRaw` rows where `video_id='v1'`

This ensures referential integrity when filters select a subset of rows. **Important:** Filters must reference columns that exist on each feature being filtered; cascade relationships determine which dependent rows are deleted when they don't share the exact filtered column. If you need to delete all dependent features regardless of filter matches, perform the deletion without filters.
