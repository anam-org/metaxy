...
with LanceDBMetadataStore("db://prod-db") as prod_store:
    with LanceDBMetadataStore("s3://staging/metadata", fallback_stores=[prod_store]) as staging_store:
        dev_store = LanceDBMetadataStore(
            "./local-metadata",
            fallback_stores=[staging_store]
        )
```
