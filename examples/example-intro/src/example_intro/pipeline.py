...
with store.open("w"):
    ...
    n_new_videos = len(video_increment.new)
    if n_new_videos > 0:
        store.write(Video, video_increment.new)
        print(f"\n[Video] Wrote {n_new_videos} new samples")
    else:
...
with store.open("w"):
    ...
    if len(face_to_process) > 0:
        # Mock face detection: add computed columns while preserving system columns
        face_results = []
        for row in face_to_process.iter_rows(named=True):
            face_results.append(
                {
                    **row,
                    "num_faces": 2,
                    "face_embeddings_path": f"/data/faces/{row['video_id']}/embeddings.npy",
                }
            )
        store.write(FaceDetection, pl.DataFrame(face_results))
        print(f"[FaceDetection] Materialized {len(face_results)} samples")
    else:
...
with store.open("w"):
    ...
    if len(transcript_to_process) > 0:
        # Mock transcription: add computed columns while preserving system columns
        transcript_results = []
        for row in transcript_to_process.iter_rows(named=True):
            transcript_results.append(
                {
                    **row,
                    "transcript": f"Transcript for {row['video_id']}.",
                    "language": "en",
                }
            )
        store.write(AudioTranscription, pl.DataFrame(transcript_results))
        print(f"[AudioTranscription] Materialized {len(transcript_results)} samples")
    else:
...
with store.open("w"):
    ...
    if len(summary_to_process) > 0:
        # Mock summarization: add computed columns while preserving system columns
        summary_results = []
        for row in summary_to_process.iter_rows(named=True):
            summary_results.append(
                {**row, "summary_text": f"Summary of {row['video_id']}."}
            )
        store.write(Summary, pl.DataFrame(summary_results))
        print(f"[Summary] Materialized {len(summary_results)} samples")
    else:
