...
class IbisMetadataStore(MetadataStore, ABC):
    ...
    def _delete_metadata_impl(
        ...
        *,
        ...
        # Use _read_ibis_table() to get Ibis-backed frame (needed for SQL compilation)
        filtered = self._read_ibis_table(feature_key, filters=filter_list)
        if filtered is None:
