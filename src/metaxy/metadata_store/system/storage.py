...
class SystemTableStorage:
    ...
    def load_feature_definitions(
        ...
        *,
        ...
        features_df = self._read_latest_features_by_project(project_list)
    ...
    def _definitions_from_dataframe(self, features_df: pl.DataFrame) -> list[FeatureDefinition]:
        ...
        for row in features_df.iter_rows(named=True):
            feature_spec_data = row["feature_spec"]
            if isinstance(feature_spec_data, str):
                feature_spec_data = json.loads(feature_spec_data)

            feature_schema_data = row["feature_schema"]
            if isinstance(feature_schema_data, str):
                feature_schema_data = json.loads(feature_schema_data)

            spec = FeatureSpec.model_validate(feature_spec_data)
