...
class MetadataStore:
    ...
    def _find_readable_storage(self, key: FeatureKey) -> StorageConfig:  # noqa: ARG002
        """Find the first storage config where a handler can handle this key."""
        for sc in self._storage:
            if self._engine.can_handle(sc):
                return sc
        raise FeatureNotFoundError(f"No storage config can read feature {key.to_string()}")

    def _find_writable_storage(self, key: FeatureKey) -> StorageConfig:  # noqa: ARG002
        """Find the first storage config where a handler can handle this key."""
        for sc in self._storage:
            if self._engine.can_handle(sc):
                return sc
