...
class MetadataStore(ABC, Generic[TRef]):
    ...
    def record_feature_graph_snapshot(
        self,
        feature: FeatureKey | type[Feature],
        *,
        ...
        This should be called explicitly (e.g., in CI) before materializing a feature.
        ...
        Args:
            ...
            snapshot_id: Optional snapshot ID representing the entire feature graph state
                (typically generated by serialize_feature_graph)
        ...
        Example:
            ...
            >>> # Or with snapshot_id
            >>> store.record_feature_graph_snapshot(MyFeature, snapshot_id="snapshot_20250113_103000")
        """
        feature_key = self._resolve_feature_key(feature)
        ...
        # Get full container specs (with code_version and deps)
        # Serialize to JSON string for storage (mode='json' handles enums properly)
        containers = [c.model_dump(mode="json") for c in feature_cls.spec.containers]  # type: ignore[attr-defined]
        ...
        version_record = pl.DataFrame(
            {
                ...
                "containers": [containers_json],  # JSON string
    ...
    def serialize_feature_graph(self) -> str:
        ...
        for feature_key in sorted(
            ...
            feature_version = feature_cls.feature_version()  # type: ignore[attr-defined]
            containers = [
                c.model_dump(mode="json") for c in feature_cls.spec.containers
