...
class _PostProcessingLazyIncrement(LazyIncrement):
    ...
    def collect(self, **kwargs: Any) -> Increment:
        ...
        return Increment(
            added=self._post_process_fn(base_result.added),
            changed=self._post_process_fn(base_result.changed),
            removed=self._post_process_fn(base_result.removed),
        )
...
class IbisJsonCompatStore(JsonStructSerializerMixin, IbisMetadataStore, ABC):
    ...
    def _post_process_resolve_update_result(
        ...
        if lazy:
            ...
            return LazyIncrement(
                ...
                post_process_fn=self._post_process_polars_frame,
            )
    ...
    def _post_process_polars_frame(self, frame: FrameT) -> FrameT:
        ...
        schema = native.collect_schema() if is_lazy else native.schema
        datetime_cols = [
