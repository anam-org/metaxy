...
class IbisJsonCompatStore(JsonStructSerializerMixin, IbisMetadataStore, ABC):
    ...
    All operations stay in Ibis lazy world - no materialization to Polars!
    ...
    def _post_process_resolve_update_result(
        ...
        if lazy:
            ...
            def ensure(
                ...
                lf = self._ensure_ibis_lazy_frame(frame)
                lf = self._ensure_struct_from_flattened(lf, METAXY_PROVENANCE_BY_FIELD)
                lf = self._ensure_struct_from_flattened(
                    lf, METAXY_DATA_VERSION_BY_FIELD
                )
                return lf
    ...
    def _post_process_polars_frame(self, frame: FrameT) -> FrameT:
        ...
        native = add_struct_column(native, METAXY_PROVENANCE_BY_FIELD)
        native = add_struct_column(native, METAXY_DATA_VERSION_BY_FIELD)

        datetime_cols = [
            col
            for col, dtype in native.schema.items()
    ...
    def _preprocess_samples_for_resolve_update(
        ...
        *,
        ...
        is_lazy_input = isinstance(df, nw.LazyFrame)
        ...
        if isinstance(polars_df, pl.LazyFrame):
            polars_frame = polars_df
        elif is_lazy_input:
            polars_frame = polars_df.lazy()
        else:
            polars_frame = polars_df

        narwhals_frame = nw.from_native(polars_frame, eager_only=False)
        if is_lazy_input:
            return cast(
                FrameT,
                self._ensure_ibis_lazy_frame(cast(nw.LazyFrame[Any], narwhals_frame)),
            )
        return cast(FrameT, narwhals_frame)
