...
class JsonStructSerializerMixin:
    ...
    def _get_flattened_field_columns(
        ...
        return {
            name: DictBasedVersioningEngine._get_flattened_column_name(
                struct_name, name
            )
    ...
    def _unpack_json_columns(
        ...
        for parent_spec in feature_plan.deps or []:
            ...
            flattened_name = DictBasedVersioningEngine._get_flattened_column_name(
        ...
        if METAXY_PROVENANCE_BY_FIELD not in ibis_table.columns:
            prov_struct_fields = {
                name: ibis_table[
                    DictBasedVersioningEngine._get_flattened_column_name(
                        METAXY_PROVENANCE_BY_FIELD, name
                    )
                ]
                for name in field_names
                if DictBasedVersioningEngine._get_flattened_column_name(
                    METAXY_PROVENANCE_BY_FIELD, name
                )
    ...
    def _pack_json_columns(
        ...
        if has_prov_fields:
            ...
            ibis_table = ibis_table.mutate(**{prov_const: pack_expr})

            cols_to_drop = [
                col for col in prov_field_columns.values() if col in ibis_table.columns
            ]
            if cols_to_drop:
                ibis_table = ibis_table.drop(*cols_to_drop)
        ...
        if has_data_version_fields:
            ...
            ibis_table = ibis_table.mutate(**{data_ver_const: data_version_pack_expr})

            cols_to_drop = [
                col
                for col in data_version_field_columns.values()
                if col in ibis_table.columns
            ]
            if cols_to_drop:
                ibis_table = ibis_table.drop(*cols_to_drop)
