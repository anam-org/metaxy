    ...
    def _insert_feature_rows(
        ...
        for row in pl_df.iter_rows(named=True):
            ...
            for col_name in column_names:
                ...
                if isinstance(source_schema[col_name], pl.Struct):
                    raise TypeError(
                        f"Column '{col_name}' is a Polars Struct but PostgreSQL target column "
                        "is JSON/JSONB. Enable auto_cast_struct_for_jsonb=True or pre-encode the "
                        "column with struct.json_encode() before writing."
                    )
