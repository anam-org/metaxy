...
class FeatureGraph:
    ...
    def _compute_snapshot_version(
        ...
        use_definition_version: bool = False,
    ) -> str:
        """Compute a snapshot version hash for a list of features.

        Args:
            features: List of (key, definition) tuples, should be sorted by key.
            use_definition_version: If True, use feature_definition_version (spec+schema only).
                If False, use feature_version (includes dependency provenance).
        """
        if not features:
            return "empty"
        ...
        for feature_key, definition in features:
            ...
            if use_definition_version:
                version = definition.feature_definition_version
            else:
                version = self.get_feature_version(feature_key)
    ...
    def get_project_snapshot_version(self, project: str) -> str:
        ...
        return self._compute_snapshot_version(project_features, use_definition_version=True)
