...
try:
    IsNotExpr = exp.IsNot  # pyright: ignore[reportAttributeAccessIssue]
except AttributeError:  # pragma: no cover - older sqlglot versions

    class IsNotExpr(exp.Expression):
        """Fallback when sqlglot lacks a dedicated IsNot node."""
...
def _expression_to_narwhals(node: exp.Expression) -> nw.Expr:
    ...
    # IS / IS NOT operators
    if isinstance(node, (exp.Is, IsNotExpr)):
        left = node.this
...
def _is_is_not_node(node: exp.Expression) -> bool:
    ...
    return isinstance(node, IsNotExpr)
...
def _maybe_null_comparison(
    ...
    eq_like = isinstance(node, (exp.EQ, exp.NEQ, exp.Is, IsNotExpr))
    if not eq_like:
        return None

    negate = isinstance(node, (exp.NEQ, IsNotExpr))
