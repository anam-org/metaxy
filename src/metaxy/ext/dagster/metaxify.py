...
def _metaxify_spec(
    ...
    *,
    ...
    # Use dict keyed by AssetKey to avoid duplicates (AssetDep is not hashable)
    ...
    # Merge deps: existing deps take precedence, then add new metaxy deps
    # Use dict keyed by asset key to deduplicate (AssetDep is not hashable)
    merged_deps: dict[dg.AssetKey, dg.AssetDep] = {}
    for dep in spec.deps:
        merged_deps[dep.asset_key] = dep
    for key, dep in deps_to_add.items():
        if key not in merged_deps:
            merged_deps[key] = dep

    replace_attrs: dict[str, Any] = {
        "key": final_key,
        "deps": list(merged_deps.values()),
