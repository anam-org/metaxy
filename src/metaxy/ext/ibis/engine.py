...
class IbisSQLHandler(IOHandler["SQLBackend", SQLTableIdentifier]):
    ...
    def _is_table_not_found_error(e: Exception) -> bool:
        """Whether `e` indicates a missing table. Override for backend-specific errors."""
    ...
    def write(
        ...
        **kwargs: Any,
        ...
        df = self._handle_map_columns(conn, df, table_name)
    ...
    def _handle_map_columns(self, conn: SQLBackend, df: Frame, table_name: str) -> Frame:
        """Convert metaxy system Struct columns to native Map format.

        Runs after ``transform_before_write``: backends that JSON-encode these
        columns first (e.g. PostgreSQL) leave no Struct columns here, so this is
        a no-op for them; native-Map backends convert them to Map storage.
        """
    ...
    def _handle_polars_map_columns(df: Frame, map_columns: set[str]) -> Frame:
        ...
        return nw.from_native(polars_map.to_arrow(polars_df), eager_only=False)
...
class IbisStoreBackcompat(MetadataStore):
    ...
    @property
    def _backcompat_ibis_engine(self) -> IbisComputeEngine:
    ...
    def conn(self) -> SQLBackend:
        ...
        return self._backcompat_ibis_engine.conn

    @property
    def connection_params(self) -> dict[str, Any]:
        _backcompat_warn_attr("connection_params")
        return self._backcompat_ibis_engine.connection_params

    @property
    def _sql_dialect(self) -> str | None:
        return self._backcompat_ibis_engine.conn.name

    @property
    def backend(self) -> str | None:
        _backcompat_warn_attr("backend")
        return self._backcompat_ibis_engine.backend

    def _backcompat_find_ibis_sql_handler(self) -> IbisSQLHandler:
        handler = self._engine.find_handler(IbisSQLHandler)
        if handler is None:
    ...
    def ibis_type_to_polars(self, ibis_type: Any) -> Any:
        ...
        return self._backcompat_find_ibis_sql_handler().ibis_type_to_polars(ibis_type)
    ...
    def _get_filtered_ibis_lazy(
        ...
        *,
        ...
        handler = self._backcompat_find_ibis_sql_handler()
        from metaxy.metadata_store.table_ref import SQLTableIdentifier

        table_id = self._storage[0].resolve(key)
        assert isinstance(table_id, SQLTableIdentifier)
        table_name = table_id.table_name
        return handler._get_filtered_ibis_lazy(self._backcompat_ibis_engine.conn, table_name, filters=filters)
