...
class PostgreSQLSQLHandler(IbisSQLHandler):
    ...
    def write(
        ...
        **kwargs: Any,  # noqa: ARG002
        ...
        table_name = table_id.table_name
        original_pl_df = self._maps_to_structs(collect_to_polars(df), table_name)
    ...
    def transform_before_write(
        ...
        pl_df = self._maps_to_structs(collect_to_polars(df), table_name)
    ...
    def _feature_field_names(self, table_name: str) -> list[str]:
        """Struct field names for the feature backing `table_name`, from the active graph.

        Returns an empty list when the feature cannot be resolved (e.g. no active graph).
        """
        try:
            graph = current_graph()
            ...
            return []
        return [field_spec.key.to_struct_key() for field_spec in plan.feature.fields]

    def _maps_to_structs(self, pl_df: pl.DataFrame, table_name: str) -> pl.DataFrame:
        """Decompose metaxy Map columns into named Struct columns for JSON encoding.

        PostgreSQL has no native Map type; the field names come from the map keys, falling
        back to the feature's declared fields when a column carries no keys (e.g. empty frames).
        """
        from metaxy.utils._arrow_map import convert_maps_to_structs

        fallback = {col_name: self._feature_field_names(table_name) for col_name in _METAXY_STRUCT_COLUMNS}
        return convert_maps_to_structs(pl_df, list(_METAXY_STRUCT_COLUMNS), fallback_field_names=fallback)

    @staticmethod
    def _structs_to_maps(pl_df: pl.DataFrame) -> pl.DataFrame:
    ...
    def _cast_empty_system_struct_columns(
        ...
        field_names = self._feature_field_names(table_name)
        if not field_names:
            return pl_df
