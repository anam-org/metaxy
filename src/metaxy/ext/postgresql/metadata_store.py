...
class PostgreSQLMetadataStore(IbisStoreBackcompat):
    ...
    def _pg_handler(self) -> PostgreSQLSQLHandler:
        ...
        handler = self._engine.find_handler(PostgreSQLSQLHandler)
        if handler is None:
    ...
    def _conn(self) -> Any:
        ...
        return self._backcompat_ibis_engine._conn
    ...
    def transform_before_write(self, df: Frame, table_name: str) -> Frame:
        from metaxy.ext.ibis.engine import _backcompat_warn_attr

        _backcompat_warn_attr("transform_before_write")
        return self._pg_handler.transform_before_write(self._backcompat_ibis_engine.conn, df, table_name)
    ...
    def _parse_json_to_struct_columns(self, pl_df: pl.DataFrame, json_columns: list[str]) -> pl.DataFrame:
        ...
        return self._pg_handler._parse_json_to_struct_columns(pl_df, json_columns)
