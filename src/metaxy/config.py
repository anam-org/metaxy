...
class MetaxyConfig(BaseSettings):
    ...
    def get_store(
        ...
        Raises:
            ValueError: If store name not found in config
        ...
        """
        from metaxy.models.feature import metaxy_registry
        ...
        registry = registry or metaxy_registry

        # Import store class
        store_class = self._import_class(store_config.type)

        # Extract fallback_stores from config (if present)
        ...
        for fallback_name in fallback_store_names:
            fallback_stores.append(self.get_store(fallback_name, registry))

        # Instantiate store with config + registry + fallback_stores
        return store_class(
            registry=registry,
